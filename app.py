...
class ExcelProcessor:
    ...
    def extract_node_b(location_info):
        ...
        Enhanced to handle multiple pattern variations"""
        ...
        # Pattern 1: Extract from InterfaceName format with If Alias
        # Example: "InterfaceName=GigabitEthernet4/1/0 If Alias=--- 10G_LINK1_TO_GQ_X16_CEA ---"
        pattern_interface = r'If\s+Alias=---\s*(?:\d+G)?[_\s]*(?:LINK\d*|Link\s*\d*)?[_\s]*(?:TO|To|to)\s+([A-Z0-9_]+)'
        match = re.search(pattern_interface, location_str, re.IGNORECASE)
        
        if match:
            node_b = match.group(1).strip()
        else:
            # Pattern 2: Look for "To" or "to" followed by text until "=" or end
            pattern2 = r'(?:To|to)\s+([^=]+?)(?=\s*=|$)'
            ...
            if match:
                ...
                if match:
                    ...
                    # Pattern 4: If no "To" found, look for common network node patterns
                    pattern4 = r'LINK[-\s]*TO\s+([^_\s]+(?:_[^_\s]+)*)'
                    match = re.search(pattern4, location_str, re.IGNORECASE)

                    if match:
                        node_b = match.group(1).strip()
                    else:
                        # Pattern 5: Extract anything after "---" or similar separators
                        pattern5 = r'---\s*(.+?)(?=\s*\(|$)'
...
if __name__ == '__main__':
    app.run(debug=True, host='0.0.0.0', port=int(os.environ.get('PORT', 5000))), '', node_b)
